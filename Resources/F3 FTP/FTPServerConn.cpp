...
void CFTPServerConn::SetEasyMode(bool value)
{
	...
	{
		LOG("FTPServerConn","New VirtualFileBrowser");
		m_FileBrowser = new VirtualFileBrowser();
	}
	else
	{
		LOG("FTPServerConn","New FileBrowser");
    ...
    for(;;)
	{
		...
		switch(FtpCommand){
            case USER:
				...
				LOG("FTP","User %s %d %d",buf,strcmp(commandString.c_str(),"EasyUser"),commandString.length());
            ...
            case PASS:
				if (gotUser )
				{
					...
					XeCryptMd5( (const PBYTE)commandString.c_str(), strlen(commandString.c_str()), NULL, NULL, NULL, NULL, (PBYTE)bytes, 0x10 );
					GetBytesString( (BYTE*)bytes, 0x10, ftpByteStr, &outLen );
					if( strcmpi( ftpByteStr, SETTINGS::getInstance().getFtpPass().c_str() ) == 0 )
					{
			...
			case MFMT:
				if (strlen(buf) == 0) {
					...
					if (hFile == INVALID_HANDLE_VALUE) {
						...
						LOG("FTP", "%d %d SYSTEMTIME %d %d %d %d %d %d", b, gle, st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
						CloseHandle(hFile);
...
//   char repbuf[500];
	...
	LOG("FTP","Init Connection for LIST");
    if (UseCtrlConn){
	...
	for(unsigned int x=0;x<folders.size();x++)
	{
		...
		LOG("FTPSERVERCONN", "PASSIVE MODE IS BROKEN AT ROOT- THIS DEBUG MESSAGE IS A HACK FIX-  PLEASE FIX ME.  KTHX");
	}
...
CmdTypes CFTPServerConn::GetCommand(char *CmdArg) 
{
	...
	for (dwBytes=0;;dwBytes++) {
		...
		if (dw==SOCKET_ERROR || dw==0) {
			LOG("FtpServer", "Socket %d timed out", CommandSocket);
			SendReply("221 goodbye (connection timed out)");
			return INVALID_COMMAND; // Timeout
		}
		dw=recv(CommandSocket,psz,1,0);
		if (dw==SOCKET_ERROR || dw==0) {
			LOG("FtpServer", "Socket %d got a network error, disconnected?", CommandSocket);
...
{
	LOG("FTPServerConn", "TODO: remove skin operation check!!!");
	return true;
...
void CFTPServerConn::Cmd_STOR(const char *filename) 
{
	...
	{
		transferSize = 0x4000;
		LOG("FTPServerConn","Writingto flash");
	}
    ...
    for(size=1;size >= 0;){
		...
		while (size > 0 &&  Offset < transferSize)
		{
			...
			if(size < 0) {
				LOG("FTPServerConn", "read failed (recv < 0)?");
			}
			Read += size;
			Offset += size;
		}
...
/*       if (fwrite(XferBuffer, Read, 1, file) != 1){
			LOG("FTPServerConn", "fwrite failed");
            size=-1;
            break;
        }
*/		 
    ...
    if(size < 0){
		LOG("FTPServerConn","file save failed %s", filename);
        Send550Error();
    }else{
        SendReply("226 Transfer Complete");
    }
...
string CFTPServerConn::PathToFileSystemPath(string pPath)
{
	...
	Drive * drv = DrivesManager::getInstance().getDriveByMountPoint(Result);
	if( drv != NULL ) {
		if( drv->GetDriveType() == DEVICE_NETWORKSMB ) {
			Result = Result + "\\" + drv->getSharePath();
		}
	}

	LOG("FTPServerConn", "PathToFileSystemPath(string %s): %s", pPath.c_str(), Result.c_str());
...
void CFTPServerConn::Cmd_EXEC(const char *filename) 
{
	...
	LOG("FTPServerConn","FTP EXEC: filename: %s", filename);
	LOG("FTPServerConn","FTP EXEC: realpath: %s", RealPath.c_str());
