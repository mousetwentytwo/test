...
namespace SpreadsheetLight
{
    ...
    internal partial class SLTool
    {
        ...
        internal static string ToCellReference(string WorksheetName, int RowIndex, int ColumnIndex, bool IsAbsolute)
        {
            ...
            if (sWorksheetName.Length > 0) sWorksheetName = string.Format("{0}!", sWorksheetName);
        ...
        internal static string RemoveNamespaceDeclaration(string Text)
        {
            ...
            string s = Text.Replace(" xmlns:x=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"", "");
            s = s.Replace(" xmlns:a=\"http://schemas.openxmlformats.org/drawingml/2006/main\"", "");
        ...
        internal static string ToSampleDisplayFormat(double Data, string FormatCode)
        {
            ...
            string sGeneralPlaceholder = "SLGENERAL";
            // specifically, no d,m,y,t,h,s but anything that can be date-formatted is out.
            string sElapsedHourPlaceholder = "SLQWR";
            string sElapsedMinutePlaceholder = "SLWRQ";
            string sElapsedSecondPlaceholder = "SLRQW";
            ...
            for (int i = 0; i < saFormat.Length; ++i)
            {
                if (i % 2 == 0)
                {
                    ...
                    saFormat[i] = Regex.Replace(saFormat[i], "[Gg][Ee][Nn][Ee][Rr][Aa][Ll]", sGeneralPlaceholder);
                    ...
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[h\\]", string.Format("$1{0}", sElapsedHourPlaceholder));
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[mm\\]", string.Format("$1{0}", sElapsedMinutePlaceholder));
                    saFormat[i] = Regex.Replace(saFormat[i], "((^|;)\\s*[^dmy]*\\s*)\\[ss\\]", string.Format("$1{0}", sElapsedSecondPlaceholder));
                    ...
                    if (saFormat[i].IndexOfAny("dDmMyYhHsS".ToCharArray()) >= 0) bIsDate = true;
                }
                else
                {
            ...
            string sFormat = string.Join("", saFormat);
            if (bIsDate)
            {
                ...
                try
                {
                    ...
                    result = dt.ToString(sFormat);
                    result = result.Replace(sGeneralPlaceholder, sGeneralResult);
                    result = result.Replace(sElapsedHourPlaceholder, ts.TotalHours.ToString("f0", CultureInfo.InvariantCulture));
                    result = result.Replace(sElapsedMinutePlaceholder, ts.TotalMinutes.ToString("f0", CultureInfo.InvariantCulture));
                    result = result.Replace(sElapsedSecondPlaceholder, ts.TotalSeconds.ToString("f0", CultureInfo.InvariantCulture));
                }
                catch
                {
        ...
        internal static SizeF MeasureText(string Text, string FontName, double FontSize, FontStyle DrawStyle)
        {
            SizeF szf = new SizeF(0, 0);
            SizeF szfGraphics;
            float fDoubleUnderscoreWidth = 0;
            float fHeightTX = 0;
            float fHeightG = 0;
            Font usablefont = SLTool.GetUsableNormalFont(FontName, FontSize, DrawStyle);
            ...
            using (Bitmap bm = new Bitmap(4096, 2048))
            {
                using (Graphics g = Graphics.FromImage(bm))
                {
                    fDoubleUnderscoreWidth = g.MeasureString("__", usablefont).Width;
                    szfGraphics = g.MeasureString(string.Format("_{0}_", Text), usablefont);
                    szf.Width = szfGraphics.Width - fDoubleUnderscoreWidth;
                    ...
                    fHeightG = szfGraphics.Height;
                    fHeightTX = (float)TextRenderer.MeasureText(Text, usablefont).Height;
                    szf.Height = (fHeightG + fHeightTX) / 2.0f;
                }
            }
