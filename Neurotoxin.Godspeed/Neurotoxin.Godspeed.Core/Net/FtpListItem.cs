...
namespace Neurotoxin.Godspeed.Core.Net
{
    ...
    public class FtpListItem : IFtpListItem
    {
        ...
        {
            get { return m_specialPermissions; }
        ...
        private static void PostParse(string path, string buf, Parser parser, FtpListItem item)
        {
            // if this is a vax/openvms file listing
            // there are no slashes in the path name
            if (parser == (ParseVaxList))
            ...
            else
            {
                ...
                if (item.LinkTarget != null && !item.LinkTarget.StartsWith("/"))
                    item.LinkTarget =
                        path.GetFtpPath(item.LinkTarget.StartsWith("./")
                                            ? item.LinkTarget.Remove(0, 2)
                                            : item.LinkTarget);
            }
        ...
        /// </summary>
        private static Mutex m_parserLock = new Mutex();
        ...
        private static void InitParsers()
        {
            try
            {
                ...
                if (m_parsers == null) 
                    m_parsers = new List<Parser> {ParseMachineList, ParseUnixList, ParseDosList, ParseVaxList};
            }
            finally
            {
            ...
            if ((m = Regex.Match(buf, regex)).Success)
            {
                ...
                if (
                    !DateTime.TryParse(m.Groups["modify"].Value, CultureInfo.InvariantCulture,
                                       DateTimeStyles.AssumeLocal, out item.m_modified))
